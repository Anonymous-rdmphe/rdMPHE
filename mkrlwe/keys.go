...
// NewConjugationKeySet returns a new empty PublicKeySet
func NewSWKSet() *SWKSet {
	swkSet := new(SWKSet)
	swkSet.Value = make(map[string]*SWK)
	return swkSet
}

// AddConjugationKey insert new publickey into PublicKeySet with its id
func (swkSet *SWKSet) AddSWK(swk *SWK) {
	swkSet.Value[swk.ID] = swk
}
...
func NewSWK(params Parameters, id string) *SWK {
	swk := new(SWK)
	swk.ID = id
	swk.Value = NewSwitchingKey(params)
